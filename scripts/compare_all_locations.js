...
// Extraire les localités du fichier secretaires.json
const secretairesRegions = [...new Set(secretaires.map(s => s.REGIONS))];
const secretairesDepartements = [...new Set(secretaires.map(s => s.DEPARTEMENTS))];
const secretairesSousPrefectures = [...new Set(secretaires.map(s => s['SOUS-PREFECTURES']))];
...
secretairesRegions.forEach(region => {
...
secretairesDepartements.forEach(dept => {
...
secretairesSousPrefectures.forEach(sp => {
...
fs.writeFileSync('./scripts/localites_mapping.json', JSON.stringify(mappingComplet, null, 2));
console.log('Le fichier de correspondance a été sauvegardé dans ./scripts/localites_mapping.json');
